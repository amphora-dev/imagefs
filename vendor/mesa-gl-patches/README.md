...
The recipe explicitly uses native ELF TLS. Emulated TLS makes `libgallium`
export `__emutls_v.*`, while `libEGL` and `libGL` still reference the plain
`_mesa_glapi_tls_*` names and consequently fail `dlopen`. Rootfs v35 and the
device-verified DirectDraw build both use the native-TLS profile.
