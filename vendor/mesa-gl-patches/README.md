...
| `0005-zink-revert-split-read-write-barriers.patch` | Revert upstream `a63851655a0` on top of Mesa 26.2.0. Splitting combined read/write access masks into consecutive barriers makes the proprietary Adreno Vulkan driver crash on the first WineD3D/DX7 draw (`vulkan.adreno.so+0x2b9344`, access `0x428`). A device bisect identified `60ef62fba63` as good and `a63851655a0` as bad; restoring combined masks preserves all later 26.2 changes. |
