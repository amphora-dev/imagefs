...
| `0005-zink-lower-qcom-line-stipple.patch` | The proprietary Qualcomm Vulkan driver advertises native stippled line rasterization but Mesa 26.2 reaches a near-NULL fault in `vulkan.adreno.so` immediately after WineD3D applies the DX7 line-pattern state. Force Zink's existing geometry/fragment shader lowering path for this driver; Mesa Turnip and other drivers keep their upstream native path. |
