...
| `0005-zink-keep-qcom-implicit-sync.patch` | Mesa 26.2 disabled Kopper's present-time implicit synchronization for the proprietary Qualcomm Vulkan driver. On Adreno 830 this turns WineD3D/DX7 into either a white wait (`ZINK_DEBUG=nobgc`) or a near-NULL fault in `vulkan.adreno.so`. Preserve the pre-26.2 synchronization path for the blob; Mesa Turnip remains on the upstream no-implicit-sync path. |
