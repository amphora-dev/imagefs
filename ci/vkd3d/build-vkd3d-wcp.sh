#!/usr/bin/env bash
# Build pinned vkd3d-proton (x86_64 + i686 PE) inside a BuildStream sandbox and emit WCP.
set -euo pipefail
...
: "${VKD3D_COMMIT:?BuildStream must provide VKD3D_COMMIT}"
: "${VKD3D_VERSION:?BuildStream must provide VKD3D_VERSION}"

JOBS="${JOBS:-$(nproc)}"
SOURCE_DATE_EPOCH="${SOURCE_DATE_EPOCH:-0}"
PACKAGE_ROOT=/tmp/vkd3d-wcp
BUILD_ROOT=/tmp/vkd3d-build

for tool in meson ninja python3 tar xz file; do
...
for tool in \
  ...
  "$LLVM_MINGW_ROOT/bin/i686-w64-mingw32-windres" \
  "$LLVM_MINGW_ROOT/bin/x86_64-w64-mingw32-widl" \
  "$LLVM_MINGW_ROOT/bin/i686-w64-mingw32-widl"; do
  test -x "$tool" || {
...
widl = '$LLVM_MINGW_ROOT/bin/x86_64-w64-mingw32-widl'
...
widl = '$LLVM_MINGW_ROOT/bin/i686-w64-mingw32-widl'
pkg-config = 'pkg-config'
...
commit_short="${VKD3D_COMMIT:0:9}"
full_version="${VKD3D_VERSION}-${commit_short}"
wcp_name="Vkd3d-${full_version}.wcp"
...
cat > "$PACKAGE_ROOT/profile.json" <<EOF
{
  ...
  "versionCode": 0,
  "description": "Vkd3d ${full_version}",
  "files": [
...
cat > "$OUTPUT_DIR/vkd3d-wcp.env" <<EOF
FULL_VERSION=$full_version
COMMIT_FULL=$VKD3D_COMMIT
